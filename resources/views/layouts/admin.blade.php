...
<html lang=en>
    ...
    <body>
            ...
            <div class=sidebar role=navigation>
                <div class="sidebar-nav navbar-collapse collapse" aria-hidden="true" aria-expanded="false">
                    <ul class=nav id=side-menu>
                        ...
                        <li  class=@php if (isset($page) && $page == "customers"){echo "active";} @endphp><a href="{{url('/admin/customers')}}" class=material-ripple><i class=material-icons>contacts</i> Emails</a></li>
        ...
        <script !src="">
            ...
            @if($page == 'products')
                @foreach($products as $product)
                    var laodFile{{$product->id}} = function(event) {
                        document.getElementById('img-update{{$product->id}}').src = URL.createObjectURL(event.target.files[0]);
                    };
                    var laodFile1{{$product->id}} = function(event) {
                        document.getElementById('img-update{{$product->id}}1').src = URL.createObjectURL(event.target.files[0]);
                    };
                    var laodFile2{{$product->id}} = function(event) {
                        document.getElementById('img-update{{$product->id}}2').src = URL.createObjectURL(event.target.files[0]);
                    };
                    var laodFile3{{$product->id}} = function(event) {
                        document.getElementById('img-update{{$product->id}}3').src = URL.createObjectURL(event.target.files[0]);
                    };
                @endforeach
            @endif
        ...
        @if($page == 'products')
            ...
            @if(!session('modal_id'))
                @if($errors->has('name') || $errors->has('image') || $errors->has('price') || $errors->has('status') || $errors->has('description') || $errors->has('image'))
                <script type="text/javascript" charset="utf-8" async defer>
                    $(window).load(function(){
                        $("#newimage").modal("show");
                    });
        ...
        @if ($errors->has('state') || $errors->has('shipping_cost'))
            <script>
                $(window).load(function(){
                    $('#newstate').modal('show');
                });
